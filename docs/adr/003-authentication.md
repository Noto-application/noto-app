...
- **refresh-cookie на `Path=/`** (#102): серверный guard работает на `/app/*`, а
  cookie с узким `Path=/api/auth/refresh` туда не долетает — тогда restore
  структурно невозможен. Поэтому refresh-cookie ставится на `Path=/`; безопасность
  держится на `HttpOnly` + `SameSite=lax` + `Secure`. (Прежний узкий path,
  зафиксированный в `auth.spec.md`, заменён — он ломал restore из этого ADR.)
  Старая узкая cookie вычищается при `login`/`refresh`/`logout`.
- **Транзиентная ошибка refresh ≠ logout** (#102): guard разлогинивает только
  при явном `401` от `/auth/refresh`; при 5xx/сетевой ошибке сессия сохраняется
  (возвращаем 503), чтобы временная недоступность не выкидывала пользователя.
