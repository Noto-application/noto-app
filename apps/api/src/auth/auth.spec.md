...
JWT с тем же jti. Replay валиден, только пока **непосредственный** successor
ещё `"1"`; после следующей ротации (`jti2` → `jti3`) старый `jti1` даёт 401,
не replay (на реальном клиенте cookie уже обновлена, сценарий искусственный).
После окна ключ истекает, старый jti → 401 **без отзыва семьи** (новый jti
остаётся жив). Logout в окне инвалидирует и старый ключ, и successor (новый
jti из кэша). Краденый старый токен в пределах grace получит ту же валидную
пару, что и легитимный клиент — врождённое свойство идемпотентного replay,
не баг.
...
- **Ротация refresh — Redis allow-list.** Каждый `/refresh` выдаёт новую пару и
  ...
  - refresh — атомарно: новый ключ `"1"`, **старый ключ на ~10s = JSON пары
    ...
    В JSON — полностью подписанные access+refresh JWT: иначе replay не отдаст
    тот же `Set-Cookie` (переподпись меняет `iat`). Tokens-at-rest ~10s — ок
    для MVP, фиксируем явно.
  - replay grace, только пока непосредственный successor = `"1"`; после
    следующей ротации старый jti → 401, не цепочка replay
