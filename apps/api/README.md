...
## Изолированные e2e (#121)

Нужен запущенный Docker с Compose v2. После установки зависимостей соберите
`pnpm --filter @noto/shared build`, затем запустите:

```bash
pnpm --filter @noto/api test:e2e
pnpm --filter @noto/api test:e2e:repeat
# Только один suite:
pnpm --filter @noto/api test:e2e auth.e2e-spec.ts
# Воспроизвести порядок из лога:
pnpm --filter @noto/api test:e2e --seed=123
```

Runner создаёт отдельный Compose project с временными Postgres 16 и Redis 7,
случайными локальными портами и без рабочих volumes. Сам применяет миграции,
фиксирует настройки auth/CORS и удаляет контейнеры, сеть и volumes после тестов,
ошибки или обычной отмены (Ctrl+C). Рабочие DATABASE_URL/REDIS_URL и `.env` не
используются. После принудительного завершения через SIGKILL окружение можно
удалить вручную: `docker compose -f apps/api/test/compose.e2e.yml -p <имя-из-лога> down -v`.

Suite идут последовательно, полный reset Postgres и Redis выполняется перед
каждым suite и тестом с данными. Одновременные запуски получают разные сервисы.
HTTP-сервер слушает один случайный порт на весь suite и закрывается через
`app.close()`, вместо повторного открытия и закрытия на каждом запросе Supertest.
`test:e2e:repeat` выполняет пять полных прогонов с разными seed, без повтора
упавших тестов; CI использует эту же команду. Прямой запуск Jest заблокирован,
чтобы случайно не очистить рабочие данные.

CORS-падение от локального origin подтверждено; гипотеза о гонках между suite
в прежнем последовательном прогоне не подтверждена. Новое окружение исключает
влияние dev-сервера и других запусков на тестовую БД и Redis.
